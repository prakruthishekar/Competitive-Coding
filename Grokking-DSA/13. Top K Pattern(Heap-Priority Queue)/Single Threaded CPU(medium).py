...
# You have a single-threaded CPU that can process at most one task at a time and will act 
...
def getOrder(tasks):
    for i, t in enumerate(tasks):  # appending the index to tasks
        t.append(i)
    
    tasks.sort(key = lambda t : t[0]) # sort the taskes based on enqueueTimei 
    # we should put the values in heap based on the enqueueTimei 

    res, minHeap = [], []
    i = 0 # current task added to the queue 
